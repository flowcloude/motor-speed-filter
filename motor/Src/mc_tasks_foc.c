...
__weak void FOC_Init(void)
{
  ...
  /* USER CODE END MCboot 0 */
    ...
    SpeedFilter_Init();   /* 初始化4种滤波器，同时运行，默认低通滤波作用于速度控制 */
      ...
      switch (Mci[M1].State)
      {
        ...
        case RUN:
        {
          ...
          else
          {
            ...
            /* 速度滤波：4种滤波同时运行，将选中的滤波结果写入速度环 */
            STO_PLL_M1._Super.hAvrMecSpeedUnit = SpeedFilter_UpdateAll(STO_PLL_M1._Super.hAvrMecSpeedUnit);
